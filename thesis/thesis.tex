    ...
    \subsection{Deriving Conjugate Gradient from Conjugate Directions}
        ...
        \subsubsection{Conjugate Gradient}
            ...
            \begin{proof}
                ...
                \end{align}
                From (3.3.42) to (3.3.43) we used $p_k = (I - \overline{P}_kA)r_k$, in addition we also make use of \hyperref[prop:Residuals_are_Orthogonal_to_P_k]{proposition 3.4} from (3.3.43) to (3.3.44). 
            \end{proof}
            ...
            \begin{proof}
                \begin{align}
                    ...
                    0
                \end{align}
                The first line is from the reccurrence of CDM residuals, and then next we make use of $a_k$ from \hyperref[prop:CDM_Recurrence]{Proposition 3.7(CDM Recurrence)} together with \hyperref[lemma:CG_Lemma_1]{Lemma 3.3.2}. Next we consider: 
                \begin{align}
            ...
            \begin{proof}
                ...
                \end{align}
                We make use of the definition for $a_{k-1}$ for the CDM algorithm (\hyperref[prop:CDM_Recurrence]{proposition 3.7} together with \hyperref[lemma:CG_Lemma_2]{lemma 3.3.2}). At this point, we have proven the short CG recurrences for $p_k$. 
            \end{proof}
...
\section{Effects of Floating Point Arithematic}
    ...
    \subsection{Ghost Eigenvalues and Losing Orthogonality}
        ...
        \subsubsection{Losing Orthogonality on Ritz Vectors}
            ...
            \par
            However, under floating point arithematic, once the Ritz vector $s_i^{(k)}$ can be spanned by the Krylov subspace with small error, it will be the case that the projection of the latest Lanczos vector onto the ritz vector begins to grow. On the plot showed in \hyperref[fig:4]{figure 4}, we projected the log absolute value of $q_k^TQ_ks_i^{(k)}$ for $i=1, 2, 3$ for all $k=20, \cdots, 64$, and we used the same setup from the last section where we demonstrated ghost eigenvalues. For comparison, \hyperref[fig:5]{figure 5} showed us the what happen in exact arithematic. One very important observation to make from \hyperref[fig:5]{figure 5} is that the peak of the blue curve, projection onto the largest ritz value happens around iteration 3.8, and around that exact same iteration in \hyperref[fig:2]{figure 2} is when the second largest eigenvalue of $T_k$ decided to shifts over to the blue curve. Bear in mind that this is in a log plot, and without the log it looks like a sharp spike. 
