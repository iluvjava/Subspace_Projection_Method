...
\section{Analysis of Conjugate Gradient and Lanczos Iterations}
    ...
    \subsection{Terminations Conditions of RACG}
        ...
        \begin{enumerate}
            \item [1.)] $k = \text{grade}(A|r_0)$ is reached.  
            \item [2.)] Or the case that $k = n + 1$
        \end{enumerate}
        Next, we wanna see that when the grade is reached, there exists a monic polynomial that interpolates the eigenvalues of matrix $A$ when the grade of the Krylov Subspace is reached, which is exactly the same as the results of the bound we derived but with $r_0$. Recalled from \hyperref[theorem:2]{Theorem 2} that when the grade of Krylov Subspace $A$ wrt $v$ is reached, then for all element $x$ in $\mathcal K_k(A|v)$, the polynomial representing $x$ has to have a non-zero constant. Using it we can get: 
