...
\section{Foundations}
    ...
    \subsection{Krylov Subspace}
        ...
        \subsubsection{The Grade and Matrix Polynomial}
            \begin{theorem}
                Let $k$ be the grade of Krylov Subspace $A$ initialized with $v$, then exists $p_{k + 1}(A|w)v$ with $w\neq \mathbf 0$, and it must be the case that $w_0\neq 0$.
            \end{theorem}
            \begin{proof}
                For contradiction suppose otherwise that such a polynomial exists then: 
                \begin{align}
                    \exists w &\neq \mathbf 0 : p_{k + 1}(A|w)v = \mathbf 0 
                    \\
                    \implies& w_0v + \sum_{j = 1}^{k} w_jA^jv = \mathbf 0
                    \\
                    \mathbf 0 &=\sum_{j = 1}^{k} w_jA^jv
                    \\
                    \mathbf 0 &= A\sum_{j = 0}^{k - 1} w_{j + 1}A^jv
                    \\
                    \implies \sum_{j = 0}^{k - 1} w_{j + 1}A^jv &= \mathbf 0 
                \end{align}
                From the second line to the third, I susbstitute $w_0 = 0$ for contradiction. On the last line, it suggest that $k$ is not the smallest, and $k - 1$ is actually the grade, contradicting the assumption that $k$ is the grade of the Krylov Subspace. Therefore, $w_0 \neq 0 $. 
            \end{proof}
...
\section{Analysis of Conjugate Gradient and Lanczos Iterations}
    ...
    \subsection{Terminations Conditions of RACG}
        Under exact arithememtic, the algorithm terminates at most $n$ iterations where $n$ is the size of the matrix $A$. This is true due to the \hyperref[theorem:CG_and_Krylov_Subspace]{CG and Krylov Subpsace}, the \hyperref[theorem:Existence_of_Grade_for_a_Krylov_Subspace]{Existence of Grade for a Krylov Subspace}. However, this bound is true for all definite matrix $A$, however, there are conditions where the termination of the CG algorithm comes early and it depends on the grade of $\mathcal K_k(A|r_0)$, which depends on the initial guess $r_0$ and the eigenvalues of matrix $A$ and we make the claim: 
        \begin{prop}
        ...
        \end{prop}
        Firstly, $\text{grade}(A|r_0)$ determines the maximum number of iterations for the algorithm because if $\mathcal K_{k + 1} = \mathcal K_k$, and the CG asserts that $r_{k + 1}\in \mathcal K_{k + 1}(A|r_0) = \mathcal K_{k}(A|r_0)$ and at the same time $r_{k + 1}\perp r_j \;\forall\; 0\le j \le k$ (recall that $r_j\;\forall 0\le j \le k$ are in $\mathcal K_k(A|r_0)$), which is impossible because we have $k +1$ orthgonal vector in the subspace $\mathcal K_k(A|r_0)$ which has only a dimension of $k$. 
        \par
        The condition: $\langle p_{k},Ap_{k}\rangle = 0$ makes the Conjugate Gradient Terminates. Because $p_{k}\in \mathcal K_{k + 1} = \mathcal K_k$ by the non-expanding krylov subspace, but $p_k\perp \text{ran}(P_k)\in \mathcal K_k$ is asserted by the conjugate gradient, which would mean that $p_k = \mathbf 0$, therefore the algorithm will inevitably terminates due to an error of dividing by zero if such a case is not handled under exact arithemtic. And when that happens, there are 2 possibilities: 
        \begin{enumerate}
            \item [1.)] $k + 1 = \text{grade}(A|r_0)$ is reached. 
            \item [2.)] Or the case that $k = n + 1$
        \end{enumerate}
        Next, we wanna see that when the grade is reached, there exists a monic polynomial the interpolates the eigenvalues of matrix $A$, which is exactly the same as the results of the bound we derived. Recalled from % TODO: 
        
        \begin{prop}
            If we know what $Q^Tr_0$ is, then the number of iterations is at most $k - m$, where $k$ is the degree of the minimal polynomial of $A$, and $m$ is the number of zeros in the vector $Q^Tr_0$. 
        \end{prop}
        This is true by considering a the index set $\mathcal I$ where $Q^Tr_0$ turns out to be zero, then above expression \hyperref[eqn:2.2.3]{(2.2.3)} can be simplified to: 
        \begin{align}
            \forall i \notin \mathcal I, w \neq \mathbf 0: p_k(\lambda_i|w)(Q^Tr_0)_i = 0
        \end{align}
        In the worst case, $\lambda_i$ are all unique for $i\in \mathcal I$, and there can be at most $k - m$ many of these eigenvalues. 
