    ...
    \subsection{Arnoldi Iterations and Lanczos}
        ...
        \subsubsection{Arnoldi Produces Orthogonal Basis for Krylov Subspace}
            ...
            \begin{remark}[Arnoldi Produces Minimal Monic in Krylov Subspace]\label{remark:Arnoldi_Produces_Minimal_Monic_in_Krylov_Subspace}
                The characteristic polynomial of $H$, minimizes $\Vert p(A|w)q_1\Vert_2$. For more information, Trefethen has a coverage on the topic in his works(\textbf{CITATION NEEDED}). The minimization property in Arnoldi translate to Lanczos Iterations as well. 
            \end{remark}
...
\section{Effects of Floating Point Arithmetic}
    ...
    \subsection{Paige's Floating Point Analysis}
        ...
        \subsubsection{Paige's Theorem and Backwards Analysis of CG}
            We now introduce a new theorem proposed by Paige (\textbf{CITATION NEEDED}). It gives a bound to the floating point errors for the Conjugate gradient by bounding the conditions number of $T_k$ from Lanczos iterations. It's stated as follows: 
