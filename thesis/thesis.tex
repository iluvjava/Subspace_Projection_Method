...
\section{Foundations}
    ...
    \subsection{Arnoldi Iterations and Lanczos}
        ...
        \subsubsection{Arnoldi Produces Orthogonal Basis for Krylov Subspace}
            ...
            \end{proof}
            \begin{remark}[Arnoldi Produces Minimal Monic in Krylov Subspace]
        ...
        \subsubsection{The Lanczos Iterations}
            ...
            \end{proof}
            \begin{remark}
                The characteristic polynomial of $T_k$ has a special minimization property. Here recall remark presented for the monic minimization properties of over Krylov subspace for the characterstic polynomial of matrix $H_k$; the Hessenberg matrix. Under Lanczos iterations the matrix $H_k$ becomes the tridiagonal $T_k$. Since matrix $A$ is symmetric, we consider its eigen decomposition in the form: $V\Lambda AV^T$, we let $\bar{p}_k(x)$ denotes the characteristic polynomial of matrix $T_k$, then using the 2 norm minimization properties we have: 
                \begin{align}
                ...
                \end{align}
                The last line is saying the conditions that the characterstic polynomial for $T_k$ from the Lanczos iterations is minimizing the weighted sum at the eigenvalues of the matrix $A$. A direct consequence of this is a more refined terminations conditions for the Lanczos iterations, giving the information for initial vector $q_1$. In addition, the $q_k$ vectors generated by lanczos minimizes over the same weighted norm at eigenvalues of $A$ and it's implicity a polynomial of $A$ multiplied by A; because $q_k\in \mathcal K_k(A|q_1)$. 
                % TODO: NOT SURE IF I SHOULD ADD THE PROOF FOR THIS PART. 
            \end{remark}
...
\section{Effects of Floating Point Arithematic}
    ...
    \subsection{Relative Errors of CG Under Floating Points}
        ...
        \begin{figure}[h]\label{fig:cg_convergence_0.9}
            \centering
            \includegraphics[width=10cm]{cg_convergence_0.9.png}
            \caption{
    ...
    \subsection{Paige's Floating Point Analysis}
        ...
        \subsubsection{Bounding the The Relative Residuals}
            ...
            \begin{remark}\label{remark:CG_Float_Remark}
                Here we present a core idea useful for the convergence rate of Conjugate Gradient under floating arithematic. The idea is that the bound for the exact arithemtic, \hyperref[theorem:CG_Convergence_Rate]{CG convergence rate} can be applied here for the term in (5.3.6): 
                $\beta_k |
                \xi_k^T T_k^{-1}\xi_1|$. 
                \par
