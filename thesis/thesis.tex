...
\section{Analysis of Conjugate Gradient and Lanczos Iterations}
    ...
    \subsection{Terminations Conditions of RACG}
        ...
        \begin{prop}
            The $\text{grade}(A|r_0)$ determines the number maximum number of iterations required before the terminations of the conjugate gradient, and by the time it terminates, the residual will be the zero vector. Further more, the upper bound for the grade of the subspace is the number of unique eigenvalues for the matrix $A$, There also exists initial guess $r_0$ where the iteration required might be shorter if it's projection onto some of the eigen vectors are zero. 
        \end{prop}
