...
\begin{center}
    ...
    Department Name: University of Washington Applied Mathematics\\
...
\newpage
\section{Foundations}
    This section focuses on important mathematical entities that are important for formulating, analyzing the Conjugate Gradient and the Lanczos Algorithm. Major parts of this section cited from... (\textbf{CITATION NEEDED})
    ...
    \subsection{Deriving Conjugate Gradient from First Principles}
        ...
        \subsubsection{CG Objective and Framework}
            ...
            \end{align}
            Refer back to \hyperref[sec:Energy_Norm_Minimization_using_Gradient]{Energy Norm Minimization using Gradient} for how to obtain the above minimization objective. Using the matrix form for the Petrov Galerkin Conditions where $W, V$ are both $P_k$, we reformulate the Norm Minimizations conditions under the framework of Petrov Galarkin conditions: 
            \begin{align}
        ...
        \subsubsection{Assisted Conjugate Gradient (Conjugate Directions)}
            ...
            \begin{definition}[Assited Conjugate Gradient]
                \begin{align}
                    \begin{cases}
            ...
            \begin{remark}[ACG is Conjugate Direction Method]
                This ACG method is nothing new, in the original paper from Hestenes and Stiefel back in 1952(\textbf{CITATION NEEDED}), they commented on the method of Conjugate Direction, for each chose of basis $\{u_i\}_{i = 1}^n$ there resides a unique algorithm. If one were to choose the basis to be the set of standard basis vector, then the resulting algorithm will be the equivalent of a Gaussian Eliminations. 
            \end{remark}
        ...
        \subsubsection{Properties of Assisted Conjugate Gradient}
            ...
            \begin{prop}[ACG Recurrence]\label{prop:ACG_Recurrences}
                \begin{align}
        ...
        \subsubsection{Residual Assisted Conjugate Gradient}
            ...
            \begin{lemma}\label{lemma:RACG_Lemma_1}
                \begin{align}
            ...
            \begin{proof}
                ...
                \end{align}
            \end{proof}
            \begin{lemma}\label{lemma:RACG_Lemma_2}
                \begin{align}
            ...
            \begin{proof}
                ...
                \begin{align}
                    ...
                    \end{bmatrix}
                \end{align}
                Next, we can make use of \hyperref[lemma:RACG_Lemma_1]{lemma 3.5.1} to get rid of $Ar_k$: 
                \begin{align}
                ...
                \end{align}
                The second line is also using the property that the matrix $A$ is symmetric, the third line is using the recurrence of the residual established for ACG (\hyperref[prop:ACG_Recurrences]{ACG Recurrences}), and the last line is true for all $0 \le j \le k - 2$ by the orhogonality of the residual proved in \hyperref[prop:1.6]{proposition 1.6}. Therefore we have: 
...
\section{Analysis of Conjugate Gradient and Lanczos Iterations}
    In this section we state the terminations conditions for the Lanczos iterations and the RACG algorithm we developed using the property of Krylov Subspace. This is just a throughly discussion about these 2 algorithm and applying the foundations and it's not following any references. 
    \subsection{Conjugate Gradient and Matrix Polynomial}
        ...
        \begin{remark}[The RACG Relative Energy Error Norm is Tight]
            The bound is tight refers to the fact that the matrix is SPD, therefore $\Vert Ax\Vert \le \Vert A\Vert\Vert x\Vert$ is tight, which justifies that (4.1.10) is tight. (4.1.12) can still be tight if we have the freedom to choose the eigenvalues of the matrix $A$. However, the bound is rarely tight. 
        \end{remark}
    ...
    \subsection{Greenbaum's Backward Analysis and Paige's Floating Point Analysis}
        ...
        \subsubsection{Bounding the The Relative Residuals}
            ...
            \begin{remark}\label{remark:CG_Float_Remark}
                Here we present the core idea behind Greenbaum's analysis for the convergence rate of Conjugate Gradient under floating arithematic. The idea is that the bound for the exact arithemtic, \hyperref[theorem:CG_Convergence_Rate]{CG convergence rate} can be applied here for the term: $\beta_k |
                \xi_k^T T_k^{-1}\xi_1|$. 
                \par
                This is true because if we were to perform an CG on the $T_k$ produced by the finite precision algorithm with the initial Lanczos vector being $\xi_1^{(n)}$, then its residual $\overline{r}_{k +1}$ would be exact and it's given as $-\beta_kT^{-1}_k\xi_kq_{k + 1}$, but with $q_{k + 1} = \xi_{k + 1}^{(n)}$, the $k + 1$ th standard basis vector in $\mathbb R^{n}$. And to our excitment, we already have the bound for $\overline{r}_{k+1}$. which eventually, proving that the CG algorithm is backwards stable. 
            \end{remark}
        ...
        \subsubsection{Convergence Rate and Paige's Theorem}
            ...
            \begin{theorem}[Paige's Theorem]
                ...
                \end{align}
            \end{theorem}
            Along with this theorem, the following quantities are also cited in Greenbaum's work (\textbf{CITATION NEEDED}) from Paige. 
            \begin{align}
            ...
            \end{align}
            Using Paige's theroem, we can bound the conditions number for the matrix $T_k$ produced by the finite precision Lanczos, which is given by: 
            \begin{align}
            ...
            \begin{prop}
                ...
                \begin{align}
                    ...
                    2 \sqrt{\tilde{k}}\left(
                        \frac{\sqrt{\tilde{k}} - 1}{\sqrt{\tilde{\kappa}} + 1}
                    \right)^k
                \end{align}    
