    ...
    \subsection{Convergence Rate of RACG under Exact Arithematic}
        ...
        \subsubsection{Uniformly Distributed Eigenvalues}
            \begin{theorem}[CG Convergence Rate]
                ...
                \begin{align}
                    \frac{\Vert e_k\Vert_A^2}{\Vert e_k\Vert_A^2}
                    \le 2 \left(
        ...
        \subsubsection{Outlier Eigenvalues}
            Using the derived theorem, we can extend it to other type of distributions of eigenvalues. Imagine one of the extreme case where some matrices that have one group of eigenvalues that are close together and one signle eigenvalue that is far away from the cluster. In that case, we can use Chebyshev differently by focusing its minimizing power across the clustered eigenvalues and use a simple polynomial to interpolate the outlier eigenvalue. 
