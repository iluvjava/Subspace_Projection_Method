...
"""
    ...
    * A lot of small eigenvalues are clustered close to zero, few larger ones are 
    ...
    returns: 
        a diagonal matrix. 
"""
function GetNastyPSDMatrix(N=20, rho::Number=0.9, inverted=false)
    @assert rho <= 1 && rho >= 0
    EigenValues = zeros(N)
...
"""
    Compute the Theoretical Error Bound or a given Linear Opeartor and the 
    iterations count that you are looking at. 
"""
function TheoreticalErrorBound(A, k)
    κ = cond(A)
